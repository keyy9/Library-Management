    ...
    if ($_SERVER['REQUEST_METHOD'] === 'POST') {
        ...
        if (empty($title) || empty($author_id) || empty($category_id) || empty($isbn)) {
            ...
            if ($stmt->fetch()) {
                ...
                // Insert new book
                $stmt = $db->prepare('INSERT INTO books (title, author_id, category_id, isbn, published_year, 
                                    publisher, description, cover_image, total_copies, available_copies) 
                                    VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?)');
                $stmt->execute([
                    $title, $author_id, $category_id, $isbn, $published_year,
                    $publisher, $description, $cover_image, $total_copies, $total_copies
                ]);
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                ...
                <div>
                    ...
                    <input type="number" id="published_year" name="published_year"
                           value="<?php echo htmlspecialchars($_POST['published_year'] ?? ''); ?>"
                ...
                <div class="col-span-2">
                    ...
                    <textarea id="description" name="description" rows="4"
                              class="w-full rounded-md border-gray-300 shadow-sm focus:border-indigo-500 focus:ring-indigo-500"><?php echo htmlspecialchars($_POST['description'] ?? ''); ?></textarea>
                </div>
            </div>
